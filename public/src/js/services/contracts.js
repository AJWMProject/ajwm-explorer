...
angular.module('insight.contracts')
	...
	.factory('Contracts',
	function(AjwmCoreLib, Opcodes, Networks, Consajwms) {

		var CONTRACT_CALL = 194;
		var CONTRACT_CREATE = 193;

		return {
			isValidAjwmAddress: function (address) {
                return AjwmCoreLib.Address.isValid(address, Consajwms.NETWORK)
			},
			getBitAddressFromContractAddress: function (contractAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        ...
                        checksum = AjwmCoreLib.crypto.Hash.sha256sha256(new AjwmCoreLib.deps.Buffer(networkId + contractAddress, 'hex')),
                        hexBitAddress = networkId + contractAddress + checksum.toString('hex').slice(0, 8);

                    return AjwmCoreLib.encoding.Base58.encode(new AjwmCoreLib.deps.Buffer(hexBitAddress, 'hex'));
            ...
            getEthAddressFromBitAddress: function (bitAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        networkId = network.pubkeyhash.toString(16),
                        hexBitAddress = AjwmCoreLib.encoding.Base58.decode(bitAddress).toString('hex');
			...
			getContractOpcodesString: function (hex) {
				...
				var contractCode = new AjwmCoreLib.deps.Buffer(hex, 'hex'),
					ops = [];
			...
			getContractBytecode: function (hex) {
				...
				try {
					...
					var script = AjwmCoreLib.Script(hex);
			...
			getContractAddress: function (txId, num) {
				var reverseTxId = txId.match(/.{2}/g).reverse().join(""),
					buf = new AjwmCoreLib.deps.Buffer(4);

				buf.writeUInt32LE(num, 0);

				var nHex = buf.toString('hex'),
					addr = reverseTxId + nHex,
					bufferAddress = AjwmCoreLib.crypto.Hash.sha256ripemd160(new AjwmCoreLib.deps.Buffer(addr, 'hex'));
