...
function($scope, $rootScope, $window, $timeout, moment, getSocket, Blocks, TransactionsByDays, Consajwms, Status, $q, StatisticsTotalSupply, MarketsInfo) {
	...
	var socket = getSocket($scope);
		...
		self.txs = [];
		self.blocks = [];
		self.chartDays = Consajwms.CHART_DAYS;
		self.rewardCurrency = Consajwms.CURRENCY.AJWM;
		self.scrollConfig = {
	...
	var _getBlocks = function() {
		...
		Blocks.get({
			limit: Consajwms.BLOCKS_DISPLAYED
		}, function(res) {
	...
	var _startSocket = function() {
		...
		socket.on('tx', function(tx) {
			...
			if (self.txs.length > Consajwms.TRANSACTION_DISPLAYED) {
				self.txs.length = Consajwms.TRANSACTION_DISPLAYED;
			}
