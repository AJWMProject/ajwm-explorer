...
function($scope, $rootScope, $document, $routeParams, $location, $window, Address, getSocket, Consajwms, ContractsInfo, Contracts, ERC20AddressBalances, ERC20ContractInfo, Web3Utils) {
    ...
    self.STORAGE_ROWS = Consajwms.STORAGE_ROWS;
    self.STORAGE_CONST = {
        STRING: 'string',
        NUMBER: 'number',
        ADDRESS: 'address',
	...
	self.qrColors = {
		background: Consajwms.QRCOLOR.background,
		color: Consajwms.QRCOLOR.color
	};

	self.balances = [];

	var _startSocket = function() {
		socket.on('ajwmd/addresstxid', function(data) {
		...
		socket.emit('subscribe', 'ajwmd/addresstxid', [addrStr]);
	};
	...
	var _stopSocket = function () {
		...
		socket.emit('unsubscribe', 'ajwmd/addresstxid', [addrStr]);
	};
            ...
            if (ethAddress) {
                ...
                ContractsInfo.get({
                    ...
                    if (info) {
                        ...
                        self.storage.viewRows = Consajwms.STORAGE_ROWS;
                    }
                }, function (e) {
                    console.log('e', e);
                });
