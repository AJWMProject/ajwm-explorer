...
function($scope, $rootScope, Currency, Consajwms, BigNumber, $filter) {

	var self = this;
	
	$rootScope.currency = {
		symbol : Consajwms.DEFAULT_CURRENCY,
		factor : 1,
		bitstamp : 0
	};
	...
	$rootScope.currency.getConvertion = function(value) {
		...
		if (!isNaN(value) && typeof value !== 'undefined' && value !== null) {
			...
			if ($rootScope.currency.symbol === Consajwms.CURRENCY.USD) {

				response = _roundFloat((value * $rootScope.currency.factor), 2);
			} 
			else if ($rootScope.currency.symbol === Consajwms.CURRENCY.mBTC) {

				$rootScope.currency.factor = 1000;
				response = _roundFloat((value * $rootScope.currency.factor), 5);
			} 
			else if ($rootScope.currency.symbol === Consajwms.CURRENCY.bits) {
	...
	self.setCurrency = function(currency) {
		...
		$rootScope.currency.symbol = Consajwms.CURRENCY[ currency ];
		localStorage.setItem('insight-currency', Consajwms.CURRENCY[ currency ]);
